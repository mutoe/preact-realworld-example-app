...
describe('# Create Article', () => {
	it('should set button disabled when title, description or body is empty', () => {
	...
	it('should send request when title, description and body are filled', () => {
	...
	it('should jump to article detail page after submit success', async () => {
	...
	it('should be send tags as string array', () => {
		const wrapper = mount(<EditArticle />);
		setInputValue(wrapper, titleInputSelector, 'title');
		setInputValue(wrapper, descriptionInputSelector, 'description');
		setInputValue(wrapper, bodyInputSelector, 'body');
...
describe('# Edit article', () => {
	it('should be fetch exist article when component did mount', () => {
		const article = generateArticles();
	...
	it('should fill form after fetch exist form', async () => {
		const article = generateArticles();
		getArticleMock.mockResolvedValue(article);
	...
	it('should send put request when form submitted', async () => {
		const article = generateArticles();
		getArticleMock.mockResolvedValue(article);
		const wrapper = mount(<EditArticle slug={article.slug} />);
		await new Promise(r => setImmediate(r));
